...
if get_user_count() == 0:
    ...
    else: tab_calendar = None
    tab_family = tabs[tab_idx]; tab_idx += 1
    if user['rolle'] == 'Admin':
        tab_admin = tabs[tab_idx]; tab_idx += 1
    else: tab_admin = None

    if 'selected_event_team' not in st.session_state: st.session_state['selected_event_team'] = None
            ...
            for _, ev in events_to_show.iterrows():
                ...
                with st.expander(f"🏐 {ev['titel']} ({ev['start_zeit']})", expanded=False):
                    ...
                    if not attendance_df.empty:
                        ...
                        if dabei: st.success(f"✅ **Dabei ({len(dabei)}):**\n" + "\n".join([f"- {name}" for name in dabei]))
                        if abgesagt: st.error(f"❌ **Abgesagt ({len(abgesagt)}):**\n" + "\n".join([f"- {name}" for name in abgesagt]))
                    else: st.info("Noch keine Rückmeldungen für diesen Spieltag.")
                    ...
                    if not ev_tasks.empty:
                        for _, tsk in ev_tasks.iterrows():
                            ...
                            st.divider()
                    else: st.info("Noch keine Aufgaben für dieses Event hinterlegt.")
